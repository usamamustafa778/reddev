...
export default function AppDevelopment() {
  ...
  // Smooth Spring Transition for Hovers
        ...
        <Container className="relative py-16 md:py-20">
          ...
          >
            <div className="inline-flex items-center gap-2 bg-white px-4 py-2 rounded-2xl border border-gray-200/50 mb-6 shadow-sm">
              ...
              <span className="text-red-600 text-sm font-semibold">Mobile Solutions</span>
            </div>
            ...
            <p className="text-gray-600 text-lg md:text-xl leading-relaxed max-w-3xl mx-auto mb-10">
              We build intuitive, high-performance iOS and Android applications. From concept to App Store launch.
            </p>
          ...
          >
            {[
              { title: "iOS & Android", desc: "Native and Cross-platform development experts.", icon: LuSmartphone, color: "from-red-500 to-red-600" },
              { title: "Seamless UI/UX", desc: "User-centric designs for maximum retention.", icon: LuAppWindow, color: "from-orange-500 to-red-500" },
              { title: "Cloud Integration", desc: "Secure and real-time data synchronization.", icon: LuLayers, color: "from-red-600 to-orange-600" },
            ].map((feature, index) => (
              <motion.div
                key={index}
                variants={fadeInUp}
          ...
          {/* Banner Section - Optimized for Smoothness */}
<motion.div 
  initial={{ opacity: 0, y: 40 }}
  whileInView={{ opacity: 1, y: 0 }}
  transition={{ duration: 0.8, ease: "easeOut" }}
  viewport={{ once: true }}
  className="relative rounded-[2.5rem] overflow-hidden shadow-2xl mt-16 group isolate" // isolate added for performance
>
  {/* Overlay - Transition optimization */}
  ...
  {/* Image Container - This ensures smooth scaling */}
  <div className="overflow-hidden w-full h-[550px] relative">
    <motion.div
      whileHover={{ scale: 1.1 }}
      transition={{ duration: 1.5, ease: [0.33, 1, 0.68, 1] }} // Custom Bezier for premium feel
      className="w-full h-full will-change-transform" // will-change is key!
    >
      <Image
        className="w-full h-full object-cover"
        src="https://images.unsplash.com/photo-1512941937669-90a1b58e7e9c?q=80&w=2070"
        alt="Mobile App Development"
        fill // 'fill' use karein smooth resizing ke liye
        priority
        sizes="100vw"
      />
    </motion.div>
  ...
  <div className="absolute bottom-0 left-0 p-10 md:p-16 z-30 w-full">
    ...
    >
      ...
      <span className="text-red-500 uppercase tracking-[0.2em] text-xs font-black">Cutting Edge Tech</span>
    </motion.div>
    
    <h3 className="text-white text-3xl md:text-5xl font-bold mb-8 max-w-2xl leading-[1.1]">
      Bring your app idea to life with our engineers.
    </h3>
    ...
    <motion.button 
      ...
      className="group flex items-center gap-3 px-8 py-4 bg-white text-red-600 rounded-full font-bold shadow-xl active:shadow-inner"
    >
